...
const narrativeIds=['thermal_crack','water_taste','sleeping_beacon','beacon_return','missing_crate','crate_manifest','outer_rattle','heater_cycle','operator_note','reserve_valve','repeater_window','repeater_reply','snow_compass'];
const narrativeFailures=[]; for(const id of narrativeIds) {
  for(const field of ['title','text']) { const key=`event.${id}.${field}`; for(const lang of ['es','ko']) if(!t[lang]?.[key]||t[lang][key]===t.en?.[key]) narrativeFailures.push(`${lang}:${key}`); }
  const event=d.events.find(item=>item.id===id); for(let i=0;i<(event?.choices||[]).length;i++) for(const field of ['title','desc']) { const key=`event.${id}.choice.${i}.${field}`; for(const lang of ['es','ko']) if(!t[lang]?.[key]||t[lang][key]===t.en?.[key]) narrativeFailures.push(`${lang}:${key}`); }
}
...
const unique=[...new Set(hardcoded)]; console.log(`Missing ES keys: ${missing.es.length}`); console.log(`Missing EN keys: ${missing.en.length}`); console.log(`Missing KO keys: ${missing.ko.length}`); console.log(`Hardcoded UI strings: ${unique.length}`); console.log(`New-event narrative localization failures: ${narrativeFailures.length}`); if(unique.length) console.log(unique.join(' | ')); if(narrativeFailures.length) console.log(narrativeFailures.join(', ')); if(Object.values(missing).some(x=>x.length)||unique.length||narrativeFailures.length) process.exitCode=1;
